    ...
    if copied_type == 'file':
        if method == 'copy':
            if tocopy_loaction == 'folder':
                os.system('..\\rclone backend copyid Zard: ' + copied_id + ' \"Zard:' + folder + '/\" --drive-server-side-across-configs --progress -v')
            elif tocopy_loaction == 'id':
                os.system('..\\rclone backend copyid Zard: '  + copied_id + ' Zard,root_folder_id=' + folder + ': --drive-server-side-across-configs --progress -v')
        elif method == 'down':
            if tocopy_loaction == 'folder':
                os.system('..\\rclone backend copyid Zard: ' + copied_id + ' \"' + 'D:\\desktop\\' + folder + '/\" --drive-server-side-across-configs --progress -v')
            elif tocopy_loaction == 'path':
                os.system('..\\rclone backend copyid Zard: ' + copied_id + ' \"' + folder + '/\" --drive-server-side-across-configs --progress -v')
    if copied_type == 'folder':
        folder_name = checked_api.get('name')
        if method == 'copy':
            if tocopy_loaction == 'folder':
                os.system('..\\rclone copy Zard,root_folder_id=' + copied_id + ': \"Zard:' + folder + '/' + folder_name + '\" --drive-server-side-across-configs --progress -v --transfers=10')
            elif tocopy_loaction == 'id':
                os.system('..\\rclone copy --drive-root-folder-id ' + copied_id + ' Zard: \"Zard,root_folder_id=' + folder + ':' + folder_name + '\" --drive-server-side-across-configs --progress -v')
        elif method == 'down':
            if tocopy_loaction == 'folder':
                os.system('..\\rclone copy Zard,root_folder_id=' + copied_id + ': \"' + 'D:\\desktop\\' + folder + '\\' + folder_name + '\" --drive-server-side-across-configs --progress -v --transfers=5')
            elif tocopy_loaction == 'path':
                os.system('..\\rclone copy Zard,root_folder_id=' + copied_id + ': \"' + folder + '\\' + folder_name + '\" --drive-server-side-across-configs --progress -v --transfers=5')
